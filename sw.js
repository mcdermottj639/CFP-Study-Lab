 ...
 * on every version bump. Precached on install (only if missing) for offline-first.
 * The list is generated by scripts/sync_infographics.mjs — don't hand-edit it. */
const MEDIA_CACHE = 'fpsl-media';
const MEDIA_ASSETS = [
  /* INFOGRAPHICS-GEN-START */
  './assets/infographics/FP511-M1-Personal-Financial-Planning.png',
...
self.addEventListener('install', (event) => {
  event.waitUntil(
    caches.open(CORE_CACHE)
      ...
      .then((cache) => Promise.all(MEDIA_ASSETS.map((u) =>
        cache.match(u).then((hit) => hit ? null :
          fetch(u).then((r) => { if (r && (r.ok || r.type === "opaque")) return cache.put(u, r); })
            .catch(function(){})
        )
      )))
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(
        keys
          .filter((k) => k !== CORE_CACHE && k !== RUNTIME_CACHE && k !== MEDIA_CACHE)

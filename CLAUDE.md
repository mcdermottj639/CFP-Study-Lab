...
`fp511-ai-slides.html`, `fp512-ai-slides.html`, `fp512-m{1,2}-ai-slides.html`) — mobile-first,
...
- The 8 Kaplan decks are grounded in the College for Financial Planning® (Kaplan) FP512 course
  slides; the 4 AI decks in the NotebookLM PDFs (badged AI + a "verify figures" footer, since
  NotebookLM numbers can be wrong — the M7 Kaplan deck also corrects a source HSA-limit typo
  $8,500→$8,550). Cross-checking their genuinely-new facts into the flashcards/reader (M3
  settlement options, M7 group carve-out & §79 70/85 tests, inherited-annuity taxation, etc.)
  is a SEPARATE pending content pass.
...
- `sw.js` `VERSION` and `build_index.mjs` `APP_VERSION` should be bumped together
  (current: `v2.62.0`) on every shippable change so installed apps auto-update

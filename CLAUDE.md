...
- **Adding a course (FP513+):** author tiers for its cards in `content/card-tiers.json`, then
  re-run `node scripts/build_card_tiers.mjs`. An **untiered card passes EVERY tier filter** by
  design, so a new course is never silently hidden from study before it has been tiered.

**Tier filter in the Study tab (v2.107.0).** The tier data is surfaced as a fourth Study-tab
picker, `#studyTier` → `window.TIERF` ∈ `all | 12 | 1 | 2 | 3` ("Skip least likely" = `12`).
- **`card-tiers.js`** (generated by **`scripts/build_card_tiers.mjs`** from `content/card-tiers.json`
  — *don't hand-edit it*) sets `window.CARDTIER` = front text → tier. Loaded after `mcq-why.js` in
  `build_index.mjs`, precached in `sw.js` `CORE_ASSETS`. The generator validates tier values and
  warns on cards with no tier / tier entries matching no card (a renamed front).
- **Engine helpers** live next to `filt()` in `src/study-home.src.html`: `cardTier(c)` (returns
  1/2/3, or **0 = untiered**), `tierOk(c)` (the filter predicate — untiered always passes), and
  `tierCounts(mod)` (per-tier counts of in-scope cards).
- **Where it bites:** `flashcards.js` `buildDeck()` applies it to **both** paths — the normal deck
  and the flagged/leech (`hard`) pool — so "tier 1 only" means the same thing everywhere. It
  composes with the course + sub-module + card-status filters. `tierPass()` falls back to keeping
  the card if `tierOk` is missing, so a failed `card-tiers.js` load can never empty the deck.
- **Tiers are authored per FLASHCARD, not per MCQ**, so `studyScopeSync()` **dims** the tier picker
  outside the `flash`/`hard` modes rather than pretending it applies to question modes. `startBtn`,
  `studyScoped()`, and `studyDomain()` all read the picker into `window.TIERF`.
- The flashcard **same-day resume key** includes the tier (alongside course/sub-module/mode/filter),
  so switching tier starts a fresh deck instead of resuming a mismatched one. The "Nothing to
  study" empty state names the active tier so a narrow filter doesn't read as "this module is empty."
